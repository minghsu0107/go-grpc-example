...
# method of password protection: DES3
# DES is an encryption method
# DES3 (also called triple DES) is the same method that is ran 3 times in a row to make the encryption stronger

# Summary 
# Private files: ca.key, server.crt, server.key, server.pem
# "Share" files: ca.crt (needed by the client), server.csr (needed by the CA)
# In real world, client software—for example, browsers—include a set of trusted CA certificates
...
# Step 5: Convert the signed certificate to .pem format (server.pem) - usable by gRPC
